...
En GitHub Actions los recordatorios esperan el éxito de classify+Notion; no tienen cron independiente. Usa los mismos defaults que el script (sin `--force`).
...
Pipeline 3 (tras ingesta)  →  cierra tareas si el mensaje es de completar
Classify+sync (tras completadas)       →  aplica intencion=completar antes del aviso
Pipeline 2 (tras sync Notion)  →  avisa en Slack (próximas/Cloudflare: cada 3 días; atrasadas: cada 2)
```
