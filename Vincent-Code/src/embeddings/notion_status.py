...
Do not reuse the old Processed Transcripts "Embeddings Ready" checkbox â€”
...
# Logical name â†’ (Notion property name, type)
...
class EmbeddingStatusNotionClient:
    def __init__(
        ...
        *,
        ...
        self._cached_data_source_id: Optional[str] = None

    def _data_source_id(self) -> str:
        if self._cached_data_source_id is not None:
            return self._cached_data_source_id
        db = self.client.databases.retrieve(database_id=self.database_id)
        sources = db.get("data_sources") or []
        self._cached_data_source_id = sources[0]["id"] if sources else self.database_id
        return self._cached_data_source_id
